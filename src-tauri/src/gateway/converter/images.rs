...
use crate::gateway::models::{ImageBlock, ImageSource};
...
pub(super) fn images_option(images: Vec<ImageBlock>) -> Option<Vec<ImageBlock>> {
    if images.is_empty() {
        None
    } else {
        Some(images)
...
pub(super) async fn extract_images(client: &Client, content: Option<&Value>) -> Vec<ImageBlock> {
...
pub(super) async fn extract_image_block(client: &Client, item: &Value) -> Option<ImageBlock> {
...
pub(super) fn media_type_to_format(media_type: &str) -> Option<String> {
...
pub(super) async fn resolve_image_source(client: &Client, url: &str) -> Option<(String, String)> {
    let _ = client;
...
pub(super) fn infer_image_format_from_url(url: &str) -> Option<String> {
...
pub(super) async fn validate_remote_image_url(url: &str) -> Option<reqwest::Url> {
...
pub(super) fn encoded_image_exceeds_limit(encoded: &str) -> bool {
    encoded.len() > max_base64_len_for_bytes(MAX_IMAGE_SOURCE_BYTES)
}

pub(super) fn max_base64_len_for_bytes(max_bytes: usize) -> usize {
    max_bytes.div_ceil(3) * 4
}

pub(super) fn is_restricted_remote_ip(ip: IpAddr) -> bool {
    match ip {
        IpAddr::V4(addr) => {
            addr.is_private()
                || addr.is_loopback()
...
pub(super) fn is_ipv4_shared(addr: Ipv4Addr) -> bool {
    let octets = addr.octets();
    octets[0] == 100 && (64..=127).contains(&octets[1])
}

pub(super) fn is_ipv4_reserved(addr: Ipv4Addr) -> bool {
    let octets = addr.octets();
    octets[0] >= 240
}

pub(super) fn is_ipv6_documentation(addr: Ipv6Addr) -> bool {
