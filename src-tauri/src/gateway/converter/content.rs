use crate::gateway::models::{KiroToolResult, KiroToolResultContent, NormalizedMessage};
...
pub(super) fn extract_reasoning_content(content: Option<&Value>) -> Option<Value> {
    let content = content?;
...
pub(super) fn meaningful_optional_value(value: Option<Value>) -> Option<Value> {
    match value {
...
pub(super) fn extract_text_blocks(value: &Value, text_types: &[&str]) -> String {
    match value {
...
/// {"type": "default"}
/// ```
pub(super) fn extract_text_and_cache_control(value: &Value) -> (String, Option<Value>) {
    match value {
...
pub(super) fn extract_cache_control_from_content(content: &Value) -> Option<Value> {
    match content {
        Value::Array(items) => {
...
/// {"type": "default"}
/// ```
pub(super) fn convert_cache_control_to_cache_point(_cache_control: &Value) -> Value {
    // Kiro API 使用简化的 cache_point 格式
    // 不需要 ttl 参数，直接使用 {"type": "default"}
    json!({"type": "default"})
}

pub(super) fn extract_tool_results(content: Option<&Value>) -> Vec<KiroToolResult> {
...
pub(super) fn extract_tool_results_from_tool_message(message: &NormalizedMessage) -> Vec<KiroToolResult> {
    vec![KiroToolResult {
...
pub(super) fn clean_system_prompt(text: &str) -> String {
...
pub(super) fn join_with_newline(left: &str, right: &str) -> String {
...
pub(super) fn join_with_double_newline(left: &str, right: &str) -> String {
    match (left.is_empty(), right.is_empty()) {
        (true, true) => String::new(),
        (true, false) => right.to_string(),
        (false, true) => left.to_string(),
