use crate::gateway::models::{
    HistoryAssistantMessage, HistoryItem, HistoryUserMessage, KiroTool, KiroToolResult,
    KiroToolResultContent, KiroToolUse, NormalizedMessage, UserInputMessageContext,
};
use serde_json::{json, Value};

use super::content::{
    extract_reasoning_content, extract_text_content, join_with_newline, meaningful_optional_value,
};
use super::tools::{sanitize_tool_name, shorten_tool_name};
...
/// 4. 修复交替（插入占位消息）
/// 5. 确保以 user 结束
pub(super) fn sanitize_history(mut items: Vec<HistoryItem>) -> Vec<HistoryItem> {
    if items.is_empty() {
        return items;
    }
...
pub(super) fn merge_adjacent_messages(messages: &[&NormalizedMessage]) -> Vec<NormalizedMessage> {
...
pub(super) fn normalized_message_has_tool_results(message: &NormalizedMessage) -> bool {
...
pub(super) fn build_user_context(
...
pub(super) fn order_tool_results_like_previous_tool_uses(
...
pub(super) fn build_history_assistant_message(message: &NormalizedMessage) -> HistoryAssistantMessage {
...
pub(super) fn assistant_metadata_value(message: &NormalizedMessage, key: &str) -> Option<Value> {
    message
        .metadata
        .as_ref()
...
pub(super) fn has_empty_thinking_signature(reasoning_content: &Option<Value>) -> bool {
...
pub(super) fn extract_tool_uses(message: &NormalizedMessage) -> Option<Vec<KiroToolUse>> {
