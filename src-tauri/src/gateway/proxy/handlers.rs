//! 网关请求编排:本地端点 handler + 本地响应构造(纯搬迁)。
use super::*;

pub(super) fn build_models_response() -> Value {
...
pub(super) fn build_count_tokens_response(payload: &Value) -> Value {
    json!({ "input_tokens": estimate_payload_tokens(payload).max(1) })
}

pub(super) fn build_openai_tokens_response(payload: &Value) -> Value {
    json!({
...
pub(super) fn build_health_response() -> Value {
    json!({ "ok": true })
}

pub(super) async fn generate_local_response(
    state: RouterState,
