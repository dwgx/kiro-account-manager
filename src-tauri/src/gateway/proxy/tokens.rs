//! 网关请求编排:token 估算与 payload 裁剪(纯搬迁)。
use super::*;

pub(super) const MAX_KIRO_PAYLOAD_SIZE: usize = 450 * 1024; // 450KB - Kiro API 的 HTTP 请求大小限制（更保守）

// Token 限制的默认值（当无法从 API 获取时使用）
#[allow(dead_code)]
pub(super) const SUMMARIZATION_THRESHOLD_PERCENT: f64 = 0.55; // 55% 触发裁剪（预留更多安全空间，避免 Kiro IDE 上下文导致超限）

pub(super) const COUNT_TOKENS_SAFETY_MULTIPLIER: f64 = 1.15;

pub(super) fn estimate_payload_tokens(payload: &Value) -> usize {
...
pub(super) fn get_payload_size(payload: &Value) -> usize {
...
#[allow(dead_code)]
pub(super) fn estimate_request_tokens(messages: &[NormalizedMessage], model_id: &str) -> usize {
    let tokenizer_type = TokenizerType::from_model_id(model_id);
...
/// 返回：是否成功裁剪
#[allow(dead_code)]
pub(super) fn trim_messages_by_tokens(
...
pub(super) fn extract_plain_text(value: Option<&Value>) -> String {
    match value {
...
/// ```
#[allow(dead_code)]
pub(super) fn estimate_text_tokens(text: &str, tokenizer_type: TokenizerType) -> usize {
    if text.is_empty() {
        return 0;
    }
...
#[allow(dead_code)]
pub(super) fn estimate_generic_tokens(text: &str) -> usize {
...
#[allow(dead_code)]
pub(super) async fn get_model_max_input_tokens(model_id: &str) -> usize {
    let model_lower = model_id.to_lowercase();
...
pub(super) fn trim_kiro_payload_history(payload: &mut Value, max_bytes: usize) -> bool {
