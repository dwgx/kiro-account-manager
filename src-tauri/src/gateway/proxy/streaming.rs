//! 网关请求编排:流式 SSE 响应(EventStream 解码→SSE 转译)(纯搬迁)。
use super::*;

pub(super) fn stream_proxy_response(
    state: RouterState,
...
pub(super) async fn handle_stream_text(
...
pub(super) async fn ensure_anthropic_message_start(
    tx: &mpsc::Sender<Result<Bytes, Infallible>>,
    message_started: &mut bool,
    anthropic_id: &str,
    model: &str,
...
pub(super) async fn close_content_block(
    tx: &mpsc::Sender<Result<Bytes, Infallible>>,
...
pub(super) async fn send_data(tx: &mpsc::Sender<Result<Bytes, Infallible>>, payload: &str) -> bool {
