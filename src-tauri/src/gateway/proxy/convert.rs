//! 网关请求编排:响应体构造(Anthropic/Responses/OpenAI + citation + 流式事件片段)(纯搬迁)。
use super::*;

pub(super) fn slice_text_by_char_range(text: &str, start: usize, end: usize) -> Option<String> {
    if end < start {
        return None;
    }
...
pub(super) fn infer_citation_text(citation: &stream::AggregatedCitation, message_text: &str) -> String {
...
pub(super) fn extract_anthropic_citation_bounds(
...
pub(super) fn build_anthropic_text_citation(
    citation: &stream::AggregatedCitation,
    message_text: &str,
) -> Option<Value> {
...
pub(super) fn build_anthropic_text_citations(
...
pub(super) fn build_anthropic_citation_delta_event(
    index: usize,
    citation: &stream::AggregatedCitation,
    message_text: &str,
) -> Option<Value> {
...
pub(super) fn build_anthropic_content_blocks(
...
pub(super) fn build_anthropic_response(model: &str, aggregated: &stream::AggregatedKiroResponse) -> Value {
...
pub(super) fn build_responses_citation_annotations(citations: &[stream::AggregatedCitation]) -> Vec<Value> {
    citations
        .iter()
        .map(|citation| {
            let mut value = json!({
...
pub(super) fn build_responses_annotation_added_event(
    response_id: &str,
    message_id: &str,
    annotation: Value,
...
pub(super) fn build_responses_output_text(aggregated: &stream::AggregatedKiroResponse) -> ResponsesOutputText {
...
pub(super) fn build_responses_message_content(aggregated: &stream::AggregatedKiroResponse) -> Vec<Value> {
...
#[allow(dead_code)]
pub(super) fn build_responses_response(
    model: &str,
    aggregated: &stream::AggregatedKiroResponse,
...
pub(super) fn build_responses_response_with_ids(
    model: &str,
    aggregated: &stream::AggregatedKiroResponse,
    response_id: &str,
    message_id: &str,
...
pub(super) fn build_stream_responses_completed_event(
    model: &str,
    aggregated: &stream::AggregatedKiroResponse,
    response_id: &str,
    message_id: &str,
...
pub(super) fn build_stream_responses_function_call_arguments_done_event(
    response_id: &str,
    call_id: &str,
    arguments: &str,
) -> Value {
...
pub(super) fn build_stream_responses_output_text_done_event(response_id: &str, text: &str) -> Value {
    json!({
...
pub(super) fn build_stream_responses_reasoning_done_event(response_id: &str, text: &str) -> Value {
    json!({
