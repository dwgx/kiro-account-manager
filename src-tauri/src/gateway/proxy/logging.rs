//! 网关请求编排:请求日志与端点/前缀映射(纯搬迁)。
use super::*;

#[derive(Debug, Clone)]
pub(super) struct RequestLogContext<'a> {
...
pub(super) fn get_request_endpoint(format: ResponseFormat) -> &'static str {
    match format {
...
pub(super) fn get_client_log_prefix(format: ResponseFormat) -> &'static str {
    match format {
...
pub(super) fn get_client_log_prefix_for_endpoint(endpoint: &str) -> &'static str {
    match endpoint {
...
pub(super) fn get_client_sse_log_file(event: Option<&str>, payload: &str) -> &'static str {
    if event.is_some() {
...
pub(super) fn serialize_logged_value(value: &Value) -> String {
    serde_json::to_string_pretty(value).unwrap_or_else(|_| value.to_string())
}

/// 从原始请求体 JSON 中提取 model 字段（用于错误日志）
pub(super) fn extract_model_from_payload(payload_str: &str) -> Option<String> {
